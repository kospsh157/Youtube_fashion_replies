...
'''
문제점1. 패션 키워드는 잘 나와도, 카테고리별로 잘 나오지 않는다. 
문제점2. 빈도수 위주로만 키워드를 잡다 보니깐, 인기있는 한 종목 혹은 하나의 패션 아이템에 치우처 있다.
문제점3  일단 댓글들의 분류 자체도 잘 안된다. 

 '니트' 라는 단어가 들어가 있는 문장들이 상의에 대한 말을 하는 문장이라고 가정하는 것이 틀린것이다. 
 니트가 아니라

아니면 댓글 데이터가 부족한것잀도있다. 계속 실시간으로 댓글을 모으면 어떨까? 
