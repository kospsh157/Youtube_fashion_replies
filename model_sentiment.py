...
stopwords = ['츠', '진행', '탑', '즈', '나중', '끈', '즈 ', '짐 ', '이야기 ', '츠 ', '당장 ', '슈', '나중 ', '진행 ', '하트 ', '스타일링 ', '일본 ', '지퍼 ', '슈 ', '에스 ', '카드 ', '세인 ', '끌 ', '이야기 ', '에스 ', '끈 ', '메종 ', '길이 ', '참여', '알파카', '제목', '트렌드', '완성', '사이', '포기', '장바구니', '국내', '노력', '감기',
             ...
             '꼴', '꼴에', '추위', '춥다', '개', '단순', '개', '강아지', '게', '세터', '난리', '트렌드', '트랜드', '브라',
             '중년', '지원', '지식', '그것', '미나', '노래', '신선', '고기', '비타민',
...
# 모델 정의
model = Sequential()
model.add(Input(shape=(max_len,)))  # 입력 데이터의 형태를 명시적으로 정의합니다.
...
def sent_pred(sent):
    ...
    sent_morphs = [
        word for word in sent_morphs if word not in stopwords]  # 불용어 제거
...
# 예측 함수 테스트
score = sent_pred("뭐야 이 평점들은... 나쁘진 않지만 10점 짜리는 더더욱 아니잖아")
print("스코어:", score)
